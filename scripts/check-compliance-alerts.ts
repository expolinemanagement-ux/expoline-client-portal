...
async function notify(companyId: string, title: string, message: string) {
  const recipients = await prisma.user.findMany({ where: { active: true, OR: [
  ...
  const existing = await prisma.notification.findMany({ where: {
    userId: { in: recipients.map(r => r.id) }, type: 'COMPLIANCE_EXPIRY', message,
...
async function main() {
  ...
  const now = new Date();
  const cutoff = new Date(now.getTime() + DAYS_AHEAD * 24 * 60 * 60 * 1000);
  const personnel = await prisma.personnel.findMany({ select: { fullName: true, companyId: true, passportExpiry: true } });
  ...
  const groups = [['Visa', visas], ['Work permit', permits], ['Medical', medical], ['Insurance', insurance], ['Document', documents]] as const;
  for (const [label, records] of groups) for (const record of records) {
    const status = classify(record.expiryDate);
    if (!status || !record.personnel || (status !== RecordStatus.EXPIRED && status !== RecordStatus.EXPIRING_SOON)) continue;
