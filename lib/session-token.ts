...
type SessionPayload={sub:string;exp:number;sv:number};
...
export async function createSessionToken(userId:string,sessionVersion=0){const payload=base64url(new TextEncoder().encode(JSON.stringify({sub:userId,exp:Math.floor(Date.now()/1000)+MAX_AGE,sv:sessionVersion})));return `${payload}.${await sign(payload)}`;}
export async function verifySessionToken(token:string|undefined){if(!token)return null;const [payload,signature]=token.split('.');if(!payload||!signature)return null;const expected=await sign(payload);if(signature!==expected)return null;try{const data=JSON.parse(new TextDecoder().decode(fromBase64url(payload))) as Partial<SessionPayload>;if(!data.sub||typeof data.sv!=='number'||!Number.isInteger(data.sv)||data.exp===undefined||data.exp<Math.floor(Date.now()/1000))return null;return data as SessionPayload}catch{return null;}}
export {MAX_AGE};
