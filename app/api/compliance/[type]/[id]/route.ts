...
export async function PUT(request:Request,{params}:Params){
 const {type,id}=await params;const model=parse(type);if(!model)return NextResponse.json({error:'Invalid record type.'},{status:400});
 try{const user=await requireUser();const existing=await (prisma as any)[model].findUnique({where:{id},include:{personnel:true}});if(!existing||!canAccessCompany(user,existing.personnel.companyId))return NextResponse.json({error:'Record not found.'},{status:404});const b=await request.json();const status:Status=statuses.includes(b.status)?b.status:'PENDING';const expiryDate=b.expiryDate?new Date(b.expiryDate):null;const issueDate=b.issueDate?new Date(b.issueDate):null;let data:any={status,expiryDate,notes:b.notes?.trim()||null};if(model==='visa')data={...data,visaNumber:b.referenceNumber?.trim()||null,visaType:b.type?.trim()||'Employment',issueDate};if(model==='workPermit')data={...data,permitNumber:b.referenceNumber?.trim()||null,permitType:b.type?.trim()||'Employment',issueDate};if(model==='medicalRecord')data={...data,medicalType:b.type?.trim()||'Employment Medical',completionDate:issueDate};if(model==='insuranceRecord')data={...data,provider:b.provider?.trim()||'Demo Insurance Provider',policyNumber:b.referenceNumber?.trim()||null,coverageType:b.type?.trim()||'Employment',startDate:issueDate};const item=await (prisma as any)[model].update({where:{id},data,include:{personnel:true}});await audit(user.id,item.personnel.companyId,'UPDATE',model,id,{personnelId:item.personnelId,status:item.status});return NextResponse.json(item);}
...
export async function DELETE(_request:Request,{params}:Params){
 const {type,id}=await params;const model=parse(type);if(!model)return NextResponse.json({error:'Invalid record type.'},{status:400});
 try{const user=await requireUser();const existing=await (prisma as any)[model].findUnique({where:{id},include:{personnel:true}});if(!existing||!canAccessCompany(user,existing.personnel.companyId))return NextResponse.json({error:'Record not found.'},{status:404});await (prisma as any)[model].delete({where:{id}});await audit(user.id,existing.personnel.companyId,'DELETE',model,id,{personnelId:existing.personnelId});return NextResponse.json({ok:true});}
